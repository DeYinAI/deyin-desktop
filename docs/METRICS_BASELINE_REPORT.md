# Reasonix Metrics — Baseline Report
...
Initial baseline from integration testing and cache guard scenarios. Use as comparison for weekly beta reports (`reasonix-weekly-*.json`).
...
| Metric | Baseline | Target |
|--------|----------|--------|
| Settings Reasonix pages load | &lt;120ms | &lt;200ms |
...
1. Aggregates counters in `reasonix-metrics.json`
2. Every 7 days writes `reasonix-weekly-YYYY-Www.json`
3. Logs summary line to `deyin.log`

View current snapshot: Settings → any Reasonix page (stats cards) or IPC `reasonix.metrics()`.
