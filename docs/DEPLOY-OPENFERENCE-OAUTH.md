...
Openference repo (`openference-platform`) and pointing the packaged Deyin desktop app at it.
...
While this work was done, `openference-platform` had a large number of concurrently-modified
...
### OAuth files to commit (all under `openference-platform`)
...
## 2. Deploy
 ...
 From `openference-platform`:

```bash
# Apply the OAuth tables to the production D1 (openference-proxy). 0154 is additive only.
...
(like the legacy UI screenshots) routes `deyin://oauth/callback` back into the app and it
