# Beta Rollout Plan — Reasonix Integration
...
2. Reasonix onboard flow → Settings → Prefix cache
3. Discord `#reasonix-beta` channel (see below)
4. In-app beta feedback form (Help menu or Settings)

### Discord channel setup

1. Create `#reasonix-beta` on the Deyin Discord server
...
4. Escalation: `@reasonix-oncall` role for P0 cache/coordinator regressions

### Weekly sync template

```markdown
## Reasonix Beta Sync — YYYY-MM-DD

### Metrics (from Settings → aggregated / reasonix-metrics.json)
- Cache hit rate: __%
...
1. **Immediate**: Ship patch with all Reasonix flags forced `false` via settings migration default override
2. **Communicate**: Discord + in-app banner with rollback notice
3. **Preserve data**: Do not delete `sessions/` or `reasonix-metrics.json`
...
- **Discord**: `#reasonix-beta`
- **Weekly reports**: `reasonix-weekly-YYYY-Www.json` auto-generated in userData

## Success criteria for GA

See [REASONIX_INTEGRATION_STATUS.md](./REASONIX_INTEGRATION_STATUS.md) and [METRICS_BASELINE_REPORT.md](./METRICS_BASELINE_REPORT.md).
