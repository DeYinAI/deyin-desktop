...
| Vision: image attach (paste/drag/pick) always goes to the user's model — no client-side capability gate; the provider's own error surfaces if the model can't take images | Done (desktop + web) | `components/Composer.tsx`, `host-core/src/models.ts` |
