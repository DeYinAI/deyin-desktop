...
/* Reasonix diagnostics (main-process ring buffers, surfaced in Settings) ------- */

export interface ReasonixCacheInvalidationEntry {
  at: number;
  threadId: string;
  reasons: string[];
  prefixHash?: string;
...
export interface ReasonixCoordinatorEntry {
  at: number;
  threadId: string;
  route: string;
  reason: string;
...
export interface ReasonixFleetEntry {
  at: number;
  threadId: string;
...
export interface ReasonixEvidenceEntry {
  at: number;
  threadId: string;
  code: string;
  message: string;
...
export interface ReasonixDiagnostics {
  cache: {
    ...
    invalidationHistory: ReasonixCacheInvalidationEntry[];
    sessionHit: number;
    sessionMiss: number;
    hitRate: number;
  };
  coordinator: ReasonixCoordinatorEntry[];
  fleet: ReasonixFleetEntry[];
  evidence: ReasonixEvidenceEntry[];
}
...
export interface DeyinSettings {
  ...
  /** Reasonix onboarding modal dismissed. */
