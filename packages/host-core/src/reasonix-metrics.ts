/**
 * Aggregated Reasonix integration metrics (cache, coordinator, fleet, UI).
 * Privacy-respecting: no prompts, paths, or code — only coarse counters and rates.
 */

export interface ReasonixMetricsSnapshot {
...
export interface ReasonixWeeklyReport {
  generatedAt: string;
  weekBucket: string;
  snapshot: ReasonixMetricsSnapshot;
  notes: string[];
}

const EMPTY: ReasonixMetricsSnapshot = {
...
function deepClone(s: ReasonixMetricsSnapshot): ReasonixMetricsSnapshot {
  return JSON.parse(JSON.stringify(s)) as ReasonixMetricsSnapshot;
}

/** Persisted metrics store with weekly rollover. */
export class ReasonixMetricsStore {
  private snapshot: ReasonixMetricsSnapshot;

  constructor(private readonly persist: (data: ReasonixMetricsSnapshot) => void, initial?: ReasonixMetricsSnapshot) {
    this.snapshot = initial ? deepClone(initial) : deepClone(EMPTY);
    this.maybeRollWeek();
  }

  get(): ReasonixMetricsSnapshot {
  ...
  generateWeeklyReport(): ReasonixWeeklyReport {
    const snap = this.get();
    const notes: string[] = [];
...
export function emptyReasonixMetrics(): ReasonixMetricsSnapshot {
