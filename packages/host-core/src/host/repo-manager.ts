...
import { git, runGit, type GitResult } from "./git.js";
...
export function slugifyRepo(url: string): string {
...
/** Ensure clone parent exists. */
export function ensureCloneDir(root: string): void {
  mkdirSync(root, { recursive: true });
}

/**
 * Git-backed workflow: clone a remote, keep work on a dedicated branch, ship via merge.
 * Token lives only in this instance (never persisted).
 */
export class RepoManager {
  ...
  constructor(
    ...
    private readonly progress: (stage: RepoProgressStage, line: string) => void,
  ) {}
  ...
  private async gitCmd(args: string[]): Promise<GitResult> {
  ...
  async connect(opts: RepoConnectOptions): Promise<RepoState> {
    ...
    if (existsSync(join(this.root, ".git"))) {
      ...
      if (current && current !== url) {
        throw new Error(`This directory is already connected to ${current}.`);
      }
      this.url = url;
      this.token = opts.token ?? null;
      this.progress("connect", `Fetching ${url}…`);
      const fetchR = await this.gitCmd(["fetch", "origin", "--prune"]);
  ...
  private clone(url: string): Promise<void> {
    const args = [...this.authArgs(), "clone", "--progress", url, this.root];
  ...
  private async resumeBranch(branch: string): Promise<string> {
    ...
    let r = await this.gitCmd(["switch", branch]);
    if (!r.ok) r = await this.gitCmd(["switch", "-c", branch, `origin/${branch}`]);
  ...
  private async createWorkBranch(): Promise<string> {
    ...
    const r = await this.gitCmd(["switch", "-c", name]);
  ...
  async ship(message?: string): Promise<RepoShipResult> {
    ...
    if (isDirty(status)) {
      ...
      const commitR = await this.gitCmd(["commit", "-m", message?.trim() || "deyin: task changes"]);
    ...
    const pushR = await this.gitCmd(["push", "-u", "origin", branch]);
    if (!pushR.ok) {
      return fail(false, `Could not push ${branch}: ${pushR.stderr.trim() || "push rejected"}. Open a pull request instead.`);
    }

    this.progress("ship", `Merging ${branch} into ${base}…`);
    const switchR = await this.gitCmd(["switch", base]);
    if (!switchR.ok) return fail(false, `Could not switch to ${base}: ${switchR.stderr.trim()}`);
    const mergeR = await this.gitCmd(["merge", "--no-ff", branch, "-m", `Merge ${branch} into ${base}`]);
    if (!mergeR.ok) {
      await this.gitCmd(["merge", "--abort"]);
      await this.gitCmd(["switch", branch]);
      return fail(false, `Merge into ${base} conflicted: ${mergeR.stderr.trim()}. Resolve locally or open a pull request.`);
    }
    const pushBaseR = await this.gitCmd(["push", "origin", base]);
    await this.gitCmd(["switch", branch]);
    if (!pushBaseR.ok) {
