...
import { computerUsePermissionRules } from "@deyin/agent-core";
...
export class ComputerUseService {
  ...
  permissionRules() {
    return computerUsePermissionRules();
  }
  ...
  /** Default-deny: empty allowlist blocks all app interactions. */
  ...
  private async ensureWindowAllowed(windowId: string): Promise<void> {
    ...
    const appId = row?.app ?? "";
    if (!appId || !this.isAppAllowed(appId)) {
      throw new Error(`Window app "${appId || "unknown"}" is not on the allowlist. Add it in Settings → Computer Use.`);
    }
  }
        ...
        execute: (args) =>
          withChain(async () => {
            ...
            if (!this.isAppAllowed(appId)) {
              throw new Error(`App "${appId}" is not on the allowlist. Add apps in Settings → Computer Use.`);
            }
      ...
      },
      {
        ...
        execute: (args) =>
          withChain(async () => {
            ...
            await this.ensureWindowAllowed(windowId);
            return JSON.stringify(
              await this.ensureHost().scroll(windowId, Number(args.delta_y) || 600),
              null,
              2,
            );
          }),
