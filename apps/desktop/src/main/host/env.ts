...
import type { EnvInfo, ShellInfo } from "../../shared/types.js";
...
export async function resolveShell(shellId?: string): Promise<{ path: string; args: string[] }> {
  ...
  if (pick) return { path: pick.path, args: pick.args ?? [] };
  // Not a known id: treat it as a literal executable path.
  if (shellId) return { path: shellId, args: [] };
  return { path: platform() === "win32" ? (process.env.COMSPEC ?? "powershell.exe") : (process.env.SHELL ?? "/bin/bash"), args: [] };
}
