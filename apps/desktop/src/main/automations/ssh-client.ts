...
/**
 * Wrap the remote agent in a process group so SSH disconnect (HUP/EXIT) kills
 * the deyin child instead of leaving orphans.
 *
 * Neither the access token nor the prompt appears in the command string: both are
 * streamed over the channel's stdin (see `buildRemoteStdin`). The remote shell reads
 * the token from the first line into a non-exported shell variable and passes it to
 * `deyin` as a per-command environment entry, so the secret never reaches
 * `/proc/<pid>/cmdline` and never lands in the long-lived login shell's environment.
 * It only lives in the deyin child's environ, which the CLI requires.
 */
