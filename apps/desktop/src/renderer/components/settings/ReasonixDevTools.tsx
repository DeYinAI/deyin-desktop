...
import type { ReasonixDiagnostics } from "../../../shared/types.js";
...
export function ReasonixDevTools({ threadId, title = "Developer diagnostics" }: Props) {
  const [diag, setDiag] = useState<ReasonixDiagnostics | null>(null);
  const [busy, setBusy] = useState(false);

  const refresh = useCallback(async () => {
    if (!window.deyin.reasonix) return;
    setBusy(true);
    try {
      setDiag(await window.deyin.reasonix.diagnostics(threadId ?? undefined));
    } finally {
      setBusy(false);
    }
  }, [threadId]);
  ...
  const clearCache = async () => {
    if (!threadId || !window.deyin.reasonix) return;
    await window.deyin.reasonix.clearThreadCache(threadId);
    await refresh();
  };

  if (!window.deyin.reasonix) return null;

  return (
    <div className="reasonix-devtools">
      <div className="settings-section">{title}</div>
      <div className="reasonix-devtools__toolbar">
      ...
      {diag && (
        <>
          <div className="reasonix-devtools__panel">
            <h3>Cache prefix</h3>
            {diag.cache.prefixShape ? (
              <dl className="reasonix-kv">
                <dt>prefixHash</dt>
